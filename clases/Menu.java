...
import parcial1.clases.nuevasClases.OpcionOnOff;
...
public class Menu implements Ejecutable, Mostrable {
	...
	protected int informarCumplidas() {
		int cont = 0;
		for (OpcionDeMenu op : items) {
			if (op instanceof OpcionOnOff && ((OpcionOnOff) op).estaActivada()) {
				cont++;
			}
		}
		return cont;
